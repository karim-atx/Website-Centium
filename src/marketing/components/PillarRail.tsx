...
// Below this viewport height there isn't room to pin a full-size pane even
// after the fit-scale floor, so the rail falls back to a normal vertical
// stack instead of pinning (initPlatformPin()'s own `vh >= 620` gate).
const MIN_PIN_VH = 620;
...
export const PillarRail: React.FC<{ pillars: PillarData[]; heading: React.ReactNode }> = ({ pillars, heading }) => {
  ...
  // Drives the unpinned fallback render (vh < MIN_PIN_VH): panes stack
  // vertically at full opacity/no filter/no pointer-events lock instead of
  // being left however the sweep last posed them — the handoff's own
  // measure() resets every one of these when it disables pinning.
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const on = vh >= MIN_PIN_VH;
      if (on !== enabled) {
        enabled = on;
        setPinEnabled(on);
        sec.style.position = on ? "sticky" : "static";
        if (!on) {
          sec.style.top = "auto";
          track.style.height = "";
    ...
    <div ref={trackRef} id="platform-track" className="relative bg-white">
      ...
      >
        <div className="max-w-[1180px] mx-auto px-5 sm:px-10 w-full">
          {heading}
          <div ref={railBoxRef} className="mt-[46px]">
            <div ref={railFitRef} className="origin-top" style={{ transformOrigin: "top center" }}>
              ...
              <div ref={viewportRef} className="rounded-[26px] overflow-hidden">
                <div
                  ...
                  style={
                    pinEnabled
                      ? { flexDirection: "row", gap: GUTTER, width: `calc(${count * 100}% + ${GUTTER * (count - 1)}px)` }
                ...
                >
                  {pillars.map((p, i) => {
                    ...
                    <div
                      ...
                      style={{
                        flex: pinEnabled ? `0 0 calc((100% - ${GUTTER * (count - 1)}px) / ${count})` : "0 0 auto",
                        background: p.wash,
                        border: `3px solid ${p.ink}`,
                        padding: "clamp(12px,1.2vw,16px)",
                        pointerEvents: !pinEnabled || i === active ? "auto" : "none",
                        opacity: !pinEnabled || d === 0 ? 1 : d === 1 ? 0.5 : 0.28,
                        filter: !pinEnabled || d === 0 ? "none" : `blur(${Math.min(4, d * 2.5)}px)`,
                        transition: "opacity .45s ease,filter .45s ease",
                      }}
                      aria-hidden={!pinEnabled || i === active ? undefined : "true"}
                    >
                      <span
                        aria-hidden="true"
