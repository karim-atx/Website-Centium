            ...
            <AnimatePresence initial={false}>
              {isOpen && (
                <motion.div
                  ...
                  // Handoff (rendered/05*.html): the grid-row wrapper animates
                  // with cubic-bezier(.22,1,.36,1), but the inner opacity
                  // transition is plain CSS `ease` (≈ cubic-bezier(.25,.1,.25,1))
                  // — the two easings are literally different, not the same
                  // curve reused. Keep them split rather than sharing one value.
                  transition={{
